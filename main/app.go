...
func main() {
	...
	sink := make(chan *NFT.AircraftNFTEventMintedAircraft)
	sub, err := aircraftNFT.WatchEventMintedAircraft(nil, sink, nil, nil) // operator và aircraftId là dùng để filter.
	if err != nil {
		log.Fatal(err)
	}
